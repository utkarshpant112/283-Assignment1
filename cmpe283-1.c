 ...
 * See SDM volume 3, section 24.6.1
 */
struct capability_info procbasedsecondary[28] =
{
 ...
 * See SDM volume 3, section 24.6.1
 */
struct capability_info vmentry[13] =
{
 ...
 * See SDM volume 3, section 24.6.1
 */
struct capability_info vmexit[17] =
{
...
detect_vmx_features(void)
{
	...
	rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
        pr_info("Procbasedsecondary Controls MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(procbasedsecondary, 28, lo, hi);
	}

	 if (hi & (1 << 17))
	 {
	 rdmsr(IA32_VMX_PROCBASED_CTLS3, lo, hi);
        pr_info("Procbasedtertiary Controls MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
	 ...
	 rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
        pr_info("VMEntry Controls MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(vmentry, 13, lo, hi);

	 rdmsr(IA32_VMX_EXIT_CTLS, lo, hi);
        pr_info("VMExit Controls MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(vmexit, 17, lo, hi);
